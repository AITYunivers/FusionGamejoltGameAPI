...
var globalThis = globalThis || Function('return this')();
...
window['darkEdif'] = (window['darkEdif'] && window['darkEdif'].sdkVersion >= 20) ? window['darkEdif'] :
	...
	this['Properties'] = function(ext, edPtrFile, extVersion) {
		...
		let bytes = edPtrFile.bytes.slice(edPtrFile.pointer);
...
function CRunGamejoltGameAPI() {
	...
	function md5(string)
	{
		var hashProvider = Windows.Security.Cryptography.Core.HashAlgorithmProvider.openAlgorithm(
			Windows.Security.Cryptography.Core.HashAlgorithmNames.md5
		);
		var inputBuffer = Windows.Security.Cryptography.CryptographicBuffer.convertStringToBinary(
			string,
			Windows.Security.Cryptography.BinaryStringEncoding.utf8
		);
		var hashedBuffer = hashProvider.hashData(inputBuffer);
		var hexString = Windows.Security.Cryptography.CryptographicBuffer.encodeToHexString(hashedBuffer);
		return hexString;
	}
	...
	const httpGet = async (url, responseType, trigger) =>
	{
		...
		try
		{
        	const response = await WinJS.xhr({ url: joltBase + serializeUrl(url, this.privateKey) });
...
CServices.extend(CRunExtension, CRunGamejoltGameAPI);
